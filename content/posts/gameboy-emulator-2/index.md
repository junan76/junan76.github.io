...
Game Boy作为一款的游戏掌机, 他发售的年代, 计算机科学和电子技术都还处于早期发展阶段. 因为距离历史足够近, 当翻阅那些颇具年代感的文档, 一行一行敲出模拟器的代码时, 也终于能够让我多看清楚一点计算机科学来时的路.

从任天堂公司成立到Game Boy发布的100年间, 人类发明了电子管, 造出了第一台通用计算机, 提出了图灵机理论, 冯诺伊曼架构, 晶体管被发明出来, 集成电路变成现实, Intel发布了历史上第一款微处理器 Intel 4004, LCD显示技术开始实用化, 一系列的技术变革, 是这款游戏掌机最终能够问世的内在推动力.
...
系统中的各个模块, 在统一的时钟驱动下向前推进. CPU执行指令, PPU解析显存数据并逐行渲染, APU根据寄存器配置生成音频数据, 定时器负责计数..., 彼此之间互不干扰. 模拟器的软件实现, 可以采用下面几种策略:
...
在游戏主循环中, 以Linux平台为例, 在每一帧的开始, 模拟器核心首先调用平台代码, 获取按键输入, 按键的输入会最为模拟器的外部输入, 更新对应寄存器的状态, 在跑完PPU渲染一帧画面所需要的tick数之后, 此时游戏的画面数据已经准备好了, 平台代码开始绘制游戏画面, 并等待下一帧的开始, 直到游戏结束.
